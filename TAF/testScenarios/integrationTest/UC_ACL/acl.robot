...
${proxyAuthUrl}   ${URI_SCHEME}://${BASE_URL}:${PROXY_AUTH_PORT}
${keyUri}  /key
...
ACL001 - Send query API by external JWT token
    ...
    And Get Token
    And Set Proxy And JWT Tokens  ${jwt_token}  ${external_token}
...
Create Key
    ...
    ${headers}  Create Dictionary  Authorization=Bearer ${proxy_token}
    ${resp}  POST On Session  Proxy Auth  ${keyUri}  data=${json_payload}  headers=${headers}
    ...       expected_status=any
    Set Response to Test Variables  ${resp}
    Run keyword if  ${response} != 201  Log  ${content}

Set Proxy And JWT Tokens
    [Arguments]  ${proxy_token_value}  ${external_token_value}
    Set Test Variable  ${proxy_token}  ${proxy_token_value}
    Set Test Variable  ${jwt_token}  ${external_token_value}
